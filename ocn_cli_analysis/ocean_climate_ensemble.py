...
def main():
    ...
    def load_data_box(members, *, component, scenario, forcing,
                      variable, time_slice, lat_box, lon_box):
        """Load a CESM2 variable over the specified box for given member(s)."""
        print(f"Loading {variable} — members={members} …")
        da = _get_dataset(
            variable,
            component=component,
            scenario=scenario,
    ...
    parser = argparse.ArgumentParser(description="CESM2-LE climate variable × index analysis")
    ...
    parser.add_argument("--variable",   type=str, required=True, help="CESM2 variable name, e.g. TEMP, SALT, SSH")
    ...
    timeseries.plot_nino34_vs_oni(
        nino34_m0,
        oni_m0,
        title="Niño-3.4 vs ONI — Member 0",
    )
    ...
    if "TLAT" in da_m0.coords and "TLONG" in da_m0.coords:
        ...
        print(f"  TLAT/TLONG attached: {tlat.shape}")
    else:
